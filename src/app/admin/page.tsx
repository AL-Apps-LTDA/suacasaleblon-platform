...
function AptCard({ apt, filter, sel, year }: { apt: ApartmentSummary; filter: string; sel: number; year: number }) {
  const fm = useMemo(() => filterMonths(apt.months || [], filter, sel, year), [apt.months, filter, sel, year])
...
export default function AdminDashboard() {
  ...
  }, [filteredValid, filter, sel, year])

  const avail = availableNights(filter, sel, aptCount, year)
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex rounded-lg border border-[rgb(var(--adm-border))] overflow-hidden text-xs">
          {([{ k: 'month' as const, l: 'Mês' }, { k: 'ytd' as const, l: 'YTD' }]).map(f => (
          ...
          <button onClick={() => setFilter('all')} className={`px-2 py-2 font-medium transition-all flex items-center gap-1 ${filter === 'all' ? 'bg-[rgb(var(--adm-accent))] text-[rgb(var(--adm-accent-fg))]' : 'bg-[rgb(var(--adm-elevated))] text-[rgb(var(--adm-muted))] hover:text-[rgb(var(--adm-text))]'}`}>
            <select value={year} onChange={e => { setYear(Number(e.target.value)); setFilter('all') }}
              className="bg-transparent outline-none cursor-pointer text-xs font-medium appearance-none pr-0"
        ...
        {filteredValid.length > 0 && (
          <div className="bg-[rgb(var(--adm-surface))] border border-[rgb(var(--adm-accent)/0.20)] rounded-xl p-4">
            ...
            <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b border-[rgb(var(--adm-border))]">
              ...
              {filteredValid.map(a => {
                const f = filterMonths(a.months || [], filter, sel, year), r = sumF(f, 'receitaTotal'), d = sumF(f, 'despesaTotal'), rs = sumF(f, 'resultado'), c = sumF(f, 'managerCommission'), o = f.reduce((s, m) => s + ownerPart(m), 0)
      ...
      {!loading && !err && activeTab === 'apartamentos' && (<>
        ...
        {aptTabList.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {aptTabList.map(a => <AptCard key={a.name} apt={a} filter={aptTabEffectiveFilter} sel={aptTabEffectiveSel} year={year} />)}
          </div>
        )}
      </>)}
    </div>
