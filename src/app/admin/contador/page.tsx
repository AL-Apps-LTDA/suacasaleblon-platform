  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-[rgb(var(--adm-text))]">Contador</h1>
          <p className="text-xs text-[rgb(var(--adm-muted))] mt-0.5">Visão contábil — receitas, despesas e resultado por apartamento</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold bg-[rgb(var(--adm-elevated))] border border-[rgb(var(--adm-border))] text-[rgb(var(--adm-muted))] hover:text-[rgb(var(--adm-text))] transition-colors">
            <Download className="h-3.5 w-3.5" /> Exportar CSV
          </button>
          <div className="flex items-center gap-1.5 bg-[rgb(var(--adm-elevated))] rounded-lg border border-[rgb(var(--adm-border))] px-1">
            <button onClick={() => setSelectedMonth(m => m > 0 ? m - 1 : 11)} className="p-1.5 text-[rgb(var(--adm-muted))] hover:text-[rgb(var(--adm-text))]"><ChevronLeft className="h-4 w-4" /></button>
            <span className="text-sm font-medium text-[rgb(var(--adm-text))] min-w-[80px] text-center">{MONTHS_FULL[selectedMonth]}</span>
            <button onClick={() => setSelectedMonth(m => m < 11 ? m + 1 : 0)} className="p-1.5 text-[rgb(var(--adm-muted))] hover:text-[rgb(var(--adm-text))]"><ChevronRight className="h-4 w-4" /></button>
          </div>
        </div>
      </div>

      {/* View toggle */}
      <div className="flex rounded-lg border border-[rgb(var(--adm-border))] overflow-hidden text-xs w-fit">
        {(['resumo', 'detalhado'] as const).map(v => (
          <button key={v} onClick={() => setView(v)} className={`px-4 py-2 font-medium capitalize ${view === v ? 'bg-[rgb(var(--adm-accent))] text-[rgb(var(--adm-accent-fg))]' : 'bg-[rgb(var(--adm-elevated))] text-[rgb(var(--adm-muted))] hover:text-[rgb(var(--adm-text))]'}`}>{v}</button>
        ))}
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              ...
              { label: 'Reservas Diretas', value: totals.direct, color: 'text-[rgb(var(--adm-accent))]', icon: DollarSign },
              ...
              <div key={c.label} className="bg-[rgb(var(--adm-surface))] border border-[rgb(var(--adm-border))] rounded-xl p-4">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-[10px] text-[rgb(var(--adm-muted))] uppercase tracking-wider">{c.label}</p>
                  <c.icon className="h-3.5 w-3.5 text-[rgb(var(--adm-accent))]" />
                </div>
              ...
              <thead>
                <tr className="border-b border-[rgb(var(--adm-border))]">
                  {['Apartamento', 'Receita Airbnb', 'Ajustes', 'Reservas Diretas', 'Despesas', 'Resultado'].map(h => (
                    <th key={h} className={`py-3 px-4 text-[rgb(var(--adm-muted))] font-medium ${h === 'Apartamento' ? 'text-left' : 'text-right'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {APARTMENTS.map(apt => {
                  ...
                  const resultado = d.receita + d.adjustments + d.directRevenue - d.despesa
                  return (
                    <tr key={apt} className="border-b border-[rgb(var(--adm-border)/0.30)] hover:bg-[rgb(var(--adm-elevated)/0.50)]">
                      <td className="py-3 px-4 font-medium text-[rgb(var(--adm-text))]">
                        <div className="flex items-center gap-2"><Building2 className="h-3.5 w-3.5 text-[rgb(var(--adm-accent))]" /> Apt {apt}</div>
                      </td>
                      ...
                      <td className="py-3 px-4 text-right font-mono text-[rgb(var(--adm-accent))]">{fmtBRL(d.directRevenue)}</td>
                ...
                <tr className="border-t-2 border-[rgb(var(--adm-accent)/0.30)] font-bold">
                  <td className="py-3 px-4 text-[rgb(var(--adm-accent))]">TOTAL</td>
                  ...
                  <td className="py-3 px-4 text-right font-mono text-[rgb(var(--adm-accent))]">{fmtBRL(totals.direct)}</td>
            ...
            <div className="space-y-4">
              {airbnbTx.length > 0 && (
                <div className="bg-[rgb(var(--adm-surface))] border border-[rgb(var(--adm-border))] rounded-xl p-4">
                  <h3 className="text-sm font-semibold text-[rgb(var(--adm-text))] mb-3">Transações Airbnb ({airbnbTx.length})</h3>
                  <div className="space-y-1 max-h-[400px] overflow-y-auto">
                    {airbnbTx.map((tx: any) => (
                      <div key={tx.id} className="flex items-center justify-between text-[11px] py-1.5 border-b border-[rgb(var(--adm-border)/0.30)]">
                        <div className="flex items-center gap-2">
                          <span className="text-[rgb(var(--adm-muted))] font-mono w-12">{tx.apartment_code}</span>
                          <span className="text-[rgb(var(--adm-muted))] font-mono w-20">{tx.date}</span>
                          <span className="text-[rgb(var(--adm-text))] truncate max-w-[200px]">{tx.description || tx.guest_name || tx.type}</span>
              ...
              {expenses.length > 0 && (
                <div className="bg-[rgb(var(--adm-surface))] border border-[rgb(var(--adm-border))] rounded-xl p-4">
                  <h3 className="text-sm font-semibold text-[rgb(var(--adm-text))] mb-3">Despesas ({expenses.length})</h3>
                  <div className="space-y-1">
                    {expenses.map((e: any) => (
                      <div key={e.id} className="flex items-center justify-between text-[11px] py-1.5 border-b border-[rgb(var(--adm-border)/0.30)]">
                        <div className="flex items-center gap-2">
                          <span className="text-[rgb(var(--adm-muted))] font-mono w-12">{e.apartment_code}</span>
                          <span className="text-[rgb(var(--adm-text))]">{e.label}</span>
                          <span className="text-[10px] text-[rgb(var(--adm-muted))]">{e.category}</span>
                        </div>
